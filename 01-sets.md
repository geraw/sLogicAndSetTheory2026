...
- **קבוצות:**
  - נסמן קבוצות באמצעות סוגריים מסולסלים.
  - דוגמה: $A = \{7, 12, 3\}$.
...
- אפשר גם לתאר קבוצה כהפעלת תבנית על כל האיברים של קבוצה נתונה:

  - לדוגמה, אם $A = \{-1,1,2,3\}$ אז:

    - $\{\,-n \mid n \in A\,\} = \{-1,1,-2,-3\}$

    - $\{\,n^2 \mid n \in A\,\} = \{1,4,9\}$

- באופן כללי: עבור תבנית $f$ ו-${A}$ קבוצה, נכתוב:
  $$ f[A] = \{\, f(x) \mid x \in A \,\} $$
...
# הבהרה: משמעות השוויון

- הבהרה כללית על שוויון במתמטיקה:
  - כאשר אומרים ששני עצמים שווים (נניח $a=b$), הכוונה שהם אותו עצם ממש.
  - למשל, אם $a$ ו-$b$ הם מספרים, אז $a=b$ פירושו ש-$a$ ו-$b$ אותו מספר בדיוק.
...
# דוגמה: שוויון קבוצות - פתרונות משוואה

- נגדיר:  $A = \{ x \in \mathbb{N} \mid x^2 - 7x + 12 = 0 \}$, $B = \{ n \in \mathbb{N} \mid 2 < n < 5 \}$
  - נטען: $A = B$.

- הוכחה (בשני הכיוונים):
  - יהי $a \in A$. 
    - אז $a$ פתרון של $x^2 - 7x + 12 = 0 = (x-3)(x-4)$, 
    - ולכן $a \in \{3,4\}$. מכאן $2 < a < 5$, ולפיכך $a \in B$.

  - יהי $b \in B$. 
    - אז $b \in \mathbb{N}$ ו-$2 < b < 5$, 
    - ולכן $b \in \{3,4\}$. 
    - נבדוק: $3^2 - 7\cdot 3 + 12 = 0$ וגם $4^2 - 7\cdot 4 + 12 = 0$. לכן $b$ פתרון למשוואה, כלומר $b \in A$.

- מסקנה: $A = B$.

---

# דוגמה: הכלה ממש - מלבנים מול מרובעים עם אלכסונים שווים

- נגדיר:
  - $A$ - קבוצת כל המלבנים במישור.
  - $B$ - קבוצת כל המרובעים במישור ששני אלכסוניהם שווים באורכם.
  - נטען: $A \neq B$.

- הוכחה:
  1. אמנם כל איבר ב-$A$ שייך גם ל-$B$, כי כל מלבן הוא מרובע ששני אלכסוניו שווים באורכם.
  
  2. אבל קיים איבר ב-$B$ שאינו שייך ל-$A$: למשל, טרפז שווה-שוקיים עם אלכסונים שווים באורכם, שאינו מלבן.

- מסקנה: $A \neq B$.

<svg class="absolute top-2.75/4 left-.8/8 w-[720px] h-[160px] pointer-events-none">
  <!-- Rectangle with equal diagonals (in A and in B) -->
  <rect x="20" y="20" width="160" height="100" fill="none" stroke="#2563eb" stroke-width="2"></rect>
  <line x1="20" y1="20" x2="180" y2="120" stroke="#10b981" stroke-width="3"></line>
  <line x1="20" y1="120" x2="180" y2="20" stroke="#10b981" stroke-width="3"></line>
  <!-- Isosceles trapezoid with equal diagonals (in B \\ A) -->
  <polygon points="280,120 420,120 380,20 320,20" fill="none" stroke="#111827" stroke-width="2"></polygon>
...
- המספרים הטבעיים היו הראשונים שהופיעו כמושגים נבדלים: מספר טבעי מונה את האיברים בקבוצה.

- ההבחנה שיש "משהו משותף" בין שלושה אנשים/תפוחים/נמרים היא מן ההכללות המתמטיות הראשונות; נמצאה גם אצל בעלי חיים מסוימים.
- לאופולד קרונקר: "האל יצר את המספרים הטבעיים – כל השאר הוא יציר האדם".
- גישת פרגה: לדוגמה, 3 = קבוצת כל הקבוצות שיש בהן שלושה איברים (התאמה לקבוצה בת שלושה איברים).
  - בעיה: מביאה לפרדוקסים, כי "אוסף כל הקבוצות" אינו קבוצה (פרדוקס ראסל).
- בניית פון נוימן (הסטנדרט בתורת הקבוצות האקסיומטית): $0=\emptyset$ ושאר המספרים מוגדרים על פי הכלל $n+1 = n \cup \{n\}$
...
| סימון מפורט | סימון מקוצר | תיאור הקבוצה |
|:---|:---:|---:|
| $\{0,1,2,3,\ldots\}$ | $\mathbb{N}$ | **קבוצת המספרים הטבעיים** |
| $\{\ldots,-2,-1,0,1,2,3,\ldots\}$ | $\mathbb{Z}$ | **קבוצת המספרים השלמים** |
| $\{ \tfrac{m}{n} \mid n \neq 0,\ m\in\mathbb{Z},\ n\in\mathbb{Z} \}$ | $\mathbb{Q}$ | **קבוצת המספרים הרציונליים** |
| $\bigl\{ \displaystyle\lim_{n\to\infty} q_n \mid q_0, q_1, \ldots \in\mathbb{Q} \bigr\}$ | $\mathbb{R}$ | **קבוצת המספרים הממשיים** |
...
# הוכיחו: $\{\emptyset\} \subsetneq \{\emptyset, \{\empty\}\}$
...
- נראה כי $x \in A \setminus (B \cap C)$ אם ורק אם $x \in (A \setminus B) \cup (A \setminus C)$.
  ...
  - הראינו כי $x$ שייך לאגף שמאל אם ורק אם הוא שייך לאגף ימין, ולכן הקבוצות שוות.  
...
# טענה: קיום ויחידות של האיחוד האונרי

**טענה:** לכל קבוצה  $X$ של קבוצות, **קיימת** קבוצה **יחידה** $U$ המקיימת:

1. $\forall S \in X(S \subseteq U)$ (כל קבוצה ב-$X$ מוכלת ב-$U$)

2. $\forall T(\forall S \in X(S \subseteq T) \to U \subseteq T)$ ($U$ היא הקבוצה המינימלית עם תכונה זו)
...
- **יחידות:** נניח שקיימות שתי קבוצות $U_1, U_2$ המקיימות את שני התנאים.
  - מתנאי 1 עבור $U_1$: לכל $S \in X$ מתקיים $S \subseteq U_1$.
  - מתנאי 2 עבור $U_2$: $U_2 \subseteq U_1$.
  - באופן סימטרי: $U_1 \subseteq U_2$.
  - לכן $U_1 = U_2$. 

::after::

**מסקנה:** הקבוצה $U = \bigcup X$ היא הקבוצה היחידה המקיימת את שני התנאים.
...
# טענה: קיום ויחידות של החיתוך האונרי

**טענה:** לכל קבוצה לא ריקה $X$ של קבוצות, קיימת קבוצה **יחידה** $I$ המקיימת:

1. $\forall S \in X(I \subseteq S)$ (הקבוצה $I$ מוכלת בכל קבוצה ב-$X$)

2. $\forall T(\forall S \in X(T \subseteq S) \to T \subseteq I)$ ($I$ היא הקבוצה המקסימלית עם תכונה זו)

**הוכחה:**

::left::

- **קיום:** נגדיר $I = \bigcap X$. 
  - תנאי 1: לכל $S \in X$, ברור ש-$\bigcap X = I \subseteq S$.
  - תנאי 2: תהי $T$ קבוצה כך שלכל $S \in X$ מתקיים $T \subseteq S$. 
    - יהי $a \in T$.
    - לכל $S \in X$ מתקיים $T \subseteq S$, ולכן $a \in S$.
    - מכיוון ש-$a$ שייך לכל $S \in X$, לפי הגדרת החיתוך האונרי $a \in \bigcap X = I$.
    - לכן $T \subseteq I$.

::right::

- **יחידות:** נניח שקיימות שתי קבוצות $I_1, I_2$ המקיימות את שני התנאים.
  - מתנאי 1 עבור $I_1$: לכל $S \in X$ מתקיים $I_1 \subseteq S$.
  - מתנאי 2 עבור $I_2$: $I_1 \subseteq I_2$.
  - באופן סימטרי: $I_2 \subseteq I_1$.
  - לכן $I_1 = I_2$. 
