...
# משפט: ערך האמת תלוי רק באטומים המופיעים בפסוק 🎯

<div class="-mt-4"></div>

<div class="absolute top-50 left-20 w-60">
  <img src="/coincidence_lemma_caricature_hebrew.png" />
</div>

<small>

**משפט:** תהי $\phi$ נוסחה, ויהיו $v_1, v_2$ שתי השמות.
אם לכל אטום $p$ המופיע ב-$\phi$ מתקיים $v_1(p) = v_2(p)$, אזי $\hat{v}_1(\phi) = \hat{v}_2(\phi)$.

**הוכחה (באינדוקציה מבנית על $\phi$):**
...
1.  **בסיס:** $\phi = p$ (אטום).
    *   נתון ש-$v_1(p) = v_2(p)$.
    *   מההגדרה, $\hat{v}_1(p) = v_1(p)$ ו-$\hat{v}_2(p) = v_2(p)$. לכן $\hat{v}_1(\phi) = \hat{v}_2(\phi)$. ✓
...
3.  **צעד (בינארי):**   $\phi = \psi_1 \circ \psi_2$ (כאשר $\circ \in \{\land, \lor, \to\}$).
    ...
    *   האטומים ב-$\phi$ הם איחוד האטומים ב-$\psi_1$ וב-$\psi_2$, לכן תנאי המשפט מתקיים עבורם.
    *   הערך של $\hat{v}(\phi)$ נקבע יחידנית ע"י הערכים $\hat{v}(\psi_1)$ ו-$\hat{v}(\psi_2)$.
    *   מההנחה, ערכים אלו זהים עבור $v_1, v_2$, ולכן גם תוצאת הפעולה זהה. ✓

</v-clicks>


**משמעות:** בהינתן נוסחה $\phi$, מספיק לבדוק שורות בטבלת האמת הרלוונטיות לאטומים שלה בלבד.
...
# סמנטיקה: המרכיבים (The Components)

כדי לתת משמעות לנוסחה (אמת/שקר), עלינו לקבוע **על מי** אנחנו מדברים ו**מה** אנחנו אומרים עליהם. המבנה $M$ מספק את המידע הזה.

**המרכיבים של מבנה $M = \langle D, R^M \rangle$:**
1.  **העולם (Domain) $D$:** קבוצת האובייקטים שעליהם אנחנו מדברים.
...
**הסימון $M \models \phi[\sigma]$**
הוא: "הנוסחה $\phi$ **מתקיימת** (אמיתית) במבנה $M$ תחת ההשמה $\sigma$".

**דוגמאות:**
*   נניח הצבה: $x \mapsto 1, y \mapsto 2$  $\longleftarrow$    $M \models R(x, y)[\langle 1, 2 \rangle]$ ✅ (כי $\langle 1, 2 \rangle \in R^M$)

*   נניח הצבה: $x \mapsto 2, y \mapsto 1$ $\longleftarrow$    $M \models R(x, y)[\langle 2, 1 \rangle]$  ❌ (כי $\langle 2, 1 \rangle \notin R^M$)
...
1.  $x + 0 = x$
2.  $x + S(y) = S(x + y)$ באשר $S$ הוא העוקב הישיר.
