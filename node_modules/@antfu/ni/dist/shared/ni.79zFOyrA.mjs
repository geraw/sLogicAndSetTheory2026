...
async function getConfig() {
  if (!config) {
    ...
    if (process$1.env.NI_RUN_AGENT === "node")
      config.runAgent = process$1.env.NI_RUN_AGENT;
    if (process$1.env.NI_USE_SFW !== void 0)
      config.useSfw = process$1.env.NI_USE_SFW === "true";
...
const parseNr = (async (agent, args, ctx) => {
  if (args.length === 0)
    args.push("start");
  const runAgent = await getRunAgent();
  let runWithNode = false;
  if (runAgent === "node") {
    const [majorNodeVersion] = process$1.versions.node.split(".").map(Number);
...
async function run(fn, args, options = {}) {
  ...
  process$1.once("SIGINT", async () => {
    await proc;
    process$1.exit(proc.exitCode);
  });
  await proc;
}

export { CLI_TEMP_DIR as C, UnsupportedCommand as U, getDefaultAgent as a, getGlobalAgent as b, getRunAgent as c, getUseSfw as d, detect as e, getCommand as f, getConfig as g, parseNr as h, parseNup as i, parseNd as j, parseNun as k, parseNlx as l, parseNa as m, getCliCommand as n, run as o, parseNi as p, remove as q, runCli as r, serializeCommand as s, exclude as t, cmdExists as u, limitText as v, writeFileSafe as w, formatPackageWithUrl as x, prompts as y };
